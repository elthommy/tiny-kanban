...
`pyproject.toml` uses loose lower-bound constraints (e.g. `fastapi>=0.115`) to declare the minimum required versions. Exact versions are pinned in two lock files generated by `uv pip compile`:

- `requirements.txt` — pinned production dependencies
- `requirements-dev.txt` — pinned production + dev dependencies

Dependabot updates these lock files automatically via weekly PRs.
...
```bash
uv pip compile pyproject.toml --python .venv/bin/python -o requirements.txt
uv pip compile pyproject.toml --python .venv/bin/python --extra dev -o requirements-dev.txt
```
